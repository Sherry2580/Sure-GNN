...
def test(dataloader, model, loss_fn, device, sensitive_cols, verbose):
  ...
  with torch.no_grad():
    for X, y, X_sensitive, _ in dataloader:
      ...
      for class_idx in range(pred.size(1)):
        this_res[f'score_class_{class_idx}'] = pred[:, class_idx].type(torch.float).cpu().numpy()
  ...
  for s, l in sensitive_cols.items():
    for sval, _ in l:
      ...
      y_scores = tmp[['score_class_0', 'score_class_1', 'score_class_2', 'score_class_3','score_class_4', 'score_class_5', 'score_class_6']].values
      this_d = {'count':len(tmp),
                'accuracy':'{:2.2f}'.format(tmp['correct'].mean()),
                'macro-auc':'{:2.2f}'.format(roc_auc_score(tmp['y'], y_scores ,multi_class='ovo',average='macro',labels=range(7))),
